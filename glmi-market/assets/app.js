...
const STORE = 'campus-loop-market-v6';
...
createApp({
  setup() {
    ...
    const baseNotifications = computed(() => [
      ...
      ...(isLoggedIn.value ? [{ id: 'auth', type: 'account', text: `Signed in as ${account.value.name}.`, time: 'Today' }] : [])
    ]);
    ...
    function openAuth(mode) { authMode.value = mode; authModal.value = true; persist(); }
    function submitAuth() {
    ...
    function logout() { isLoggedIn.value = false; showProfileMenu.value = false; persist(); ping('Signed out of demo account.'); }
    ...
    return { market, page, query, category, sortBy, priceLabel, liked, filtered, selected, sellerProfile, savedItems, activeThread, inboxThread, notifications, postedItems, myActiveListings, mySoldListings, myDraft, reserveQueue, soldItems, selectedGallery, activeGalleryIndex, showNotifications, showProfileMenu, loading, sidebarOpen, recentItems, recommended, compareItems, similarItems, alertCount, sellerMetrics, showCheckout, checkoutStep, authModal, authMode, isLoggedIn, account, authForm, persist, go, setCategory, toggleLike, openListing, closeListing, openSeller, closeSeller, messageSeller, reserveItem, markSold, fakeAction, submitListing, fakeRefresh, toggleSidebar, toggleCompare, openCheckout, closeCheckout, nextCheckout, openAuth, submitAuth, logout, toast };
  },
  template: `
    <div class="market-shell">
      ...
      <div class="modal-backdrop" v-if="authModal" @click.self="authModal = false"><div class="modal auth-modal"><div class="modal-header"><div><h3>{{ authMode === 'signup' ? 'Create account' : 'Sign in' }}</h3><div class="meta">Static onboarding flow with realistic gating.</div></div><button class="close" @click="authModal = false">Close</button></div><div class="card-actions" style="margin-bottom:12px"><button class="ghost" @click="authMode = 'login'">Login</button><button class="ghost" @click="authMode = 'signup'">Register</button></div><div class="form-grid"><label v-if="authMode === 'signup'" class="full"><span>Name</span><input v-model="authForm.name" @input="persist" placeholder="Ming Zhao" /></label><label class="full"><span>Email</span><input v-model="authForm.email" @input="persist" placeholder="name@campus.nz" /></label><label class="full"><span>Password</span><input v-model="authForm.password" @input="persist" type="password" placeholder="••••••••" /></label></div><div class="card-actions"><button class="primary" @click="submitAuth">{{ authMode === 'signup' ? 'Create account' : 'Sign in' }}</button><button class="ghost" @click="authModal = false">Continue browsing as guest</button></div></div></div>
